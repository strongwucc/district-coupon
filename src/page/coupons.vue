    ...
    <div ref="couponsWrapper" class="coupons-wrapper">
      <ul class="coupons-wrapper-content">
        <li
          ...
          :key="couponIndex"
          @click.stop="showCouponDetail(coupon.id)"
        >
          <div class="li_left">
...
export default {
  ...
  methods: {
    getCoupons() {
      ...
      this.$http
        .post(this.API.coupons + "?page=" + page, postData)
        .then(res => {
      ...
      this.$http.post(this.API.autologin, { openId: this.openId }).then(res => {
        if (res.access_token) {
          ...
          this.$vux.toast.show({
            type: "text",
            text: res.message
              ? '<span style="font-size: 14px">' + res.message + "</span>"
    ...
    initScroll() {
      ...
      this.$refs.couponsWrapper.style.minHeight = `${getRect(
        this.$refs.couponsWrapper
      ).height + 1}px`;
      let options = {
        probeType: 1,
        click: true,
